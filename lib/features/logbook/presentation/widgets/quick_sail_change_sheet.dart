...
/// Rýchly záznam obratu alebo halzy počas plavby.
///
/// Papierový denník má na prehodenie plachiet ten istý riadok ako na všetko
/// ostatné — skiper doň zapíše nový kurz voči vetru. Tu je to jedno ťuknutie:
/// sheet ponúkne siluetu s naposledy zapísaným kurzom a uloží záznam s GPS
/// a časom, ktoré appka už pozná.
///
...
class _QuickSailChangeSheetState extends ConsumerState<QuickSailChangeSheet> {
  ...
  /// Predvyplní posledný zapísaný kurz dňa — po obrate sa mení spravidla len
  /// bok, takže skiper má o jedno ťuknutie menej.
  Future<void> _loadLast() async {
    ...
    final last = await ref.read(databaseProvider).lastSailDirectionForDay(dayLogId);
    if (!mounted) return;
    setState(() {
